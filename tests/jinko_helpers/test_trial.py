...
class TestGetLatestTrialWithStatus(unittest.TestCase):
    @patch("jinko_helpers.make_request")
    @patch("jinko_helpers.getCoreItemId")
    def test_get_latest_trial_with_single_status_string(
        self, mock_get_core_item_id, mock_make_request
    ):
        mock_get_core_item_id.return_value = {"id": "core-trial-id"}
        mock_response = MagicMock()
        mock_response.json.return_value = [
            {
                "status": "completed",
                "simulationId": {
                    "coreItemId": "trial-core",
                    "snapshotId": "trial-snapshot",
                },
            }
        ]
        mock_make_request.return_value = mock_response

        result = jinko.get_latest_trial_with_status("tr-ldh6-ROIN", "completed")

        self.assertEqual(
            result,
            {"coreItemId": "trial-core", "snapshotId": "trial-snapshot"},
        )
        mock_make_request.assert_called_once_with(
            "/core/v2/trial_manager/trial/core-trial-id/status",
            params={"statuses": ["completed"]},
        )

    @patch("jinko_helpers.make_request")
    @patch("jinko_helpers.getCoreItemId")
    def test_get_latest_trial_with_multiple_statuses(
        self, mock_get_core_item_id, mock_make_request
    ):
        mock_get_core_item_id.return_value = {"id": "core-trial-id"}
        mock_response = MagicMock()
        ...
        result = jinko.get_latest_trial_with_status(
            "tr-ldh6-ROIN", ["completed", "stopped"]
        )
    ...
    @patch("jinko_helpers.make_request")
    @patch("jinko_helpers.getCoreItemId")
    def test_get_latest_trial_with_status_raises_when_api_returns_no_match(
        self, mock_get_core_item_id, mock_make_request
    ):
        mock_get_core_item_id.return_value = {"id": "core-trial-id"}
        mock_response = MagicMock()
        ...
        with self.assertRaises(ValueError) as context:
            jinko.get_latest_trial_with_status("tr-ldh6-ROIN", "completed")

        self.assertEqual(
            str(context.exception),
            "Found no trial with status among ['completed']",
        )

    def test_get_latest_trial_with_status_rejects_empty_statuses(self):
        with self.assertRaises(ValueError) as context:
            jinko.get_latest_trial_with_status("tr-ldh6-ROIN", [])
